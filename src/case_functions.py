...
def case_variants(n, m, i):
    ...
    # Extract first 9+n columns and last m columns
    columns_extracted = pd.concat([df.iloc[:, :9+n].copy(), df.iloc[:, -m:]].copy(), axis=1)
    
    # Combine extracted rows and columns
    extracted_df = rows_extracted[columns_extracted.columns].copy()   

    # Instead of adding columns one-by-one, create them all at once
    if i > 0:
        new_cols = {f'new_col_{col_index + 1}': ["0/0"] * len(extracted_df)
                    for col_index in range(i)}
        extracted_df = pd.concat([extracted_df, pd.DataFrame(new_cols)], axis=1)
...
def case_genes(n, m): 
    ...
    # Extract first n rows and last m rows
    extracted_df = pd.concat([df.head(n), df.tail(m)], ignore_index=True)
    
    # Create SAMPLEID column with zero-padded numbers starting from 0001
    sample_ids = [f"{i:04d}" for i in range(1, len(extracted_df) + 1)]
    
    # Insert SAMPLEID as the first column
    extracted_df.insert(0, "SAMPLEID", sample_ids)
...
def case_phenotypes(n, m):
    ...
    # Extract first n rows and last m rows
    rows_extracted = pd.concat([df.head(n), df.tail(m)],ignore_index=True)

    # Create SAMPLEID column with zero-padded numbers starting from 0001
    sample_ids = [f"{i:04d}" for i in range(1, len(rows_extracted) + 1)]
    
    # Insert SAMPLEID as the first column
    rows_extracted.insert(0, "SAMPLEID", sample_ids)
...
def case_hpo(i, n, m):
    ...
    # Overwrite SAMPLEID values instead of inserting new column
    df_case_hposet["SAMPLEID"] = [f"{i:04d}" for i in range(1, len(df_case_hposet) + 1)]

    
    # Save the result to a csv file
    df_case_hposet.to_csv('case_hpo.csv', index=False)
