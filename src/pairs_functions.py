...
def pairs_phenotypes(n, m):
    ...
    # Save the result to a txt file
    rows_extracted.to_csv('pairs_phenotypes.csv', index=False)
...
def pairs_hpo(i, n, m):
    ...
    df_pairs_hposet["SAMPLEID"] = [f"{i:04d}" for i in range(1, len(df_pairs_hposet) + 1)]
    
    # Save the result to a csv file
    df_pairs_hposet.to_csv('pairs_hpo.csv', index=False)
