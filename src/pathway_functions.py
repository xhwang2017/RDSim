...
def pathway_variants(n, m):
    ...
    data_file = os.path.join(script_dir, "../data/variants_pathway_vcf.csv")
...
def pathway_genes(n, m): 
    ...
    # Read the file into a DataFrame
    data_file = os.path.join(script_dir, "../data/pathway_variants_gene_orpha_inher.csv")
    data_file = os.path.abspath(data_file)
    df = pd.read_csv(data_file)
...
def pathway_phenotypes(n, m):
    ...
    # Read the file into a DataFrame
    data_file = os.path.join(script_dir, "../data/pathway_orphacode_hposet.csv")
    data_file = os.path.abspath(data_file)
    df = pd.read_csv(data_file)
...
def pathway_hpo(i, n, m):
    ...
    df_pathway_hposet["SAMPLEID"] = [f"{i:04d}" for i in range(1, len(df_pathway_hposet) + 1)]
    
    # Save the result to a csv file
    df_pathway_hposet.to_csv('pathway_hpo.csv', index=False)
